...
uint32_t tab[2][3][FREQ_NUM] = {
    ...
    // low band
    {
        {0x3574, 0x35D2, 0x3631, 0x368F, 0x36ED, 0x374C, 0x37AA, 0x3809, 0x3840, 0x38A4},
        {0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x96, 0x97},
        {0x1455555, 0x132AAAB, 0x1200000, 0x10D5555, 0xFAAAAB, 0xE80000, 0xD55555, 0xC2AAAB, 0x000000, 0x155555},
    },
};
...
void DM6302_SetChannel(uint8_t ch) {
    ...
    SPI_Write(0, 0x3, 0x120, tab[is_low_band][0][ch]); // ch
    ...
    SPI_Write(0, 0x3, 0x104, tab[is_low_band][1][ch]); // ch
    SPI_Write(0, 0x3, 0x108, tab[is_low_band][2][ch]); // ch
...
void DM6302_Init2(uint8_t sel, uint8_t freq) {
    ...
    SPI_Write(sel, 0x3, 0x120, tab[is_low_band][0][freq]); // ch
    ...
    SPI_Write(sel, 0x3, 0x104, tab[is_low_band][1][freq]); // ch
    SPI_Write(sel, 0x3, 0x108, tab[is_low_band][2][freq]); // ch
