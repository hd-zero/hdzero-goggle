...
static lv_obj_t *page_power_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    create_text(NULL, section, false, _("power:"), LV_MENU_ITEM_BUILDER_VARIANT_2);
    ...
    // create menu entries
    create_label_item(cont, _("battery"), 1, ROW_BATT_C_LABEL, 1);
    label_cell_count = create_label_item(cont, "-S", 2, ROW_BATT_C_LABEL, 1);
    create_btn_group_item(&btn_group_cell_count_mode, cont, 2, _("cell_count_mode"), _("auto"), _("manual"), "", "", ROW_CELL_COUNT_MODE);
    create_slider_item(&slider_group_cell_count, cont, _("cell_count"), CELL_MAX_COUNT, g_setting.power.cell_count, ROW_CELL_COUNT);
    create_slider_item(&slider_group_cell_voltage, cont, _("warning_cell_voltage"), WARNING_CELL_VOLTAGE_MAX, g_setting.power.voltage, ROW_WARNING_CELL_VOLTAGE);
    create_slider_item(&slider_group_calibration_offset, cont, _("voltage_calibration"), 0, g_setting.power.calibration_offset, ROW_CALIBRATION_OFFSET);
    create_btn_group_item(&btn_group_osd_display_mode, cont, 2, _("display_mode"), _("total"), _("cell_average"), "", "", ROW_OSD_DISPLAY_MODE);
    create_btn_group_item(&btn_group_warn_type, cont, 3, _("warning_type"), _("beep"), _("visual"), _("both"), "", ROW_WARN_TYPE);

    // Batch 2 goggles only
    if (getHwRevision() >= HW_REV_2) {
        create_btn_group_item(&btn_group_power_ana, cont, 2, _("analogrx_power"), _("on"), _("auto"), "", "", ROW_POWER_ANA);
    }

    // Back entry
    create_label_item(cont, _("back"), 1, pp_power.p_arr.max - 1, 1);
