...
typedef struct
{
    uint8_t* dat; //sps data
    int len;            //sps data bits num
    int idx;            //current index
    int pps_pos;
} sps_bit_stream;

int bs_fine_pps_pos(uint8_t* d, int len)
{
    int i, ret = 0;
    for (i = 0; i < len - 3; i++) {
        if (d[i] == 0 && d[i + 1] == 0 && d[i + 2] == 0 && d[i + 3] == 0x01) {
            ret = (i << 3);
            break;
        }
        else
...
int bs_skip_header_sync(sps_bit_stream* bs)
{
    bs->idx += 32;

    if (bs->dat[0] != 0 || bs->dat[1] != 0 || bs->dat[2] != 0 || bs->dat[3] != 1 || bs_eof(bs))
        return 1;
    else
        return 0;
}
...
{
    int i = 0, j = 0;
    int dataSizeTemp = (bs->pps_pos) >> 3;

    for (i = 0, j = 0; i < (dataSizeTemp - 2); i++) {
        if((bs->dat[i] == 0) && (bs->dat[i + 1] == 0) && (bs->dat[i + 2] == 0x03)) {
...
void bs_add_emulation(sps_bit_stream* bs)
{
    ...
    for (i = 5; i < (bs->pps_pos >> 3); i++) {
        if (bs->dat[i - 2] == 0 && bs->dat[i - 1] == 0 && bs->dat[i] <= 0x03) {
            bs->idx = (i << 3);
        ...
        for (int matrix_id = 0; matrix_id < (size_id == 3) ? 2 : 6; matrix_id++) {
            ...
            if (!scaling_list_pred_mode_flag[size_id][matrix_id]) { // [sizeId][matrixId]
                ...
                for (int i = 0; i < coef_num; i++) {
                    next_coef = (next_coef + bs_pop_ue(bs) + 256) % 256;
...
void short_term_ref_pic_set(sps_bit_stream* bs, int stRpsIdx, int num_short_term_ref_pic_sets)
{
    ...
    uint32_t num_negative_pics[32] = { 0 };
    uint32_t num_positive_pics[32] = { 0 };
    ...
    if (inter_ref_pic_set_prediction_flag) {
        ...
        bs_pop_u(bs, 1); // delta_rps_sign
        bs_pop_ue(bs); // abs_delta_rps_minus1
        int RefRpsIdx = stRpsIdx - (delta_idx_minus1 + 1);
        int NumDeltaPocs = num_negative_pics[RefRpsIdx] + num_positive_pics[RefRpsIdx];
        for (int j = 0; j <= NumDeltaPocs; j++) {
            used_by_curr_pic_flag[j] = bs_pop_u(bs, 1);
            if (!used_by_curr_pic_flag[j])
                bs_pop_u(bs, 1); // use_delta_flag
        }
    }
    else {
        num_negative_pics[stRpsIdx] = bs_pop_ue(bs);
        num_positive_pics[stRpsIdx] = bs_pop_ue(bs);
        for (int i = 0; i < num_negative_pics[stRpsIdx]; i++) {
            bs_pop_ue(bs); // delta_poc_s0_minus1
            bs_pop_u(bs, 1); // used_by_curr_pic_s0_flag
        }
        for (int i = 0; i < num_positive_pics[stRpsIdx]; i++) {
            bs_pop_ue(bs); // delta_poc_s1_minus1
            bs_pop_u(bs, 1); // used_by_curr_pic_s1_flag
        }
    }
}
...
int h265_insert_vui(sps_bit_stream* bs, int fps)
{
    ...
    bs_insert_u(1, 32, bs); // num_units_in_tick
    bs_insert_u(fps, 32, bs); // time_scale
    bs_insert_u(0, 32, bs); // vui_poc_proportional_to_timing_flag
    bs_insert_u(0, 32, bs); // vui_hrd_parameters_present_flag
...
int H264_Modify_SPS(uint8_t* src, int srcSize, int fps)
{
    ...
    bs.len = srcSize << 3;
    bs.idx = 0;
    bs.pps_pos = bs_fine_pps_pos(bs.dat+1, srcSize);

    // remove emulation byte(s)
    bs_remove_emulation(&bs);

    // header sync: 00 00 00 01
    if (bs_skip_header_sync(&bs)) {
        printf("H264_Modify_SPS Error: wrong header sync.\n");
        return -1;
    }
    ...
    if (nal_unit_type == 0x7) { //Nal SPS Flag
        ...
        bs.idx = bs.pps_pos;
        if(bs.pps_pos % 8)
            bs_insert_u(0, 8 - (bs.pps_pos % 8), &bs);
...
int H265_Modify_SPS(uint8_t* src, int srcSize, int fps)
{
    ...
    // header sync: 00 00 00 01
    if (bs_skip_header_sync(&bs)) {
        printf("H265_Modify_SPS Error: wrong header sync.\n");
        return -1;
    }
